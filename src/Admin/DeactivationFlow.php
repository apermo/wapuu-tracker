...
namespace Apermo\WapuuTracker\Admin;
...
class DeactivationFlow {
	 ...
	 */
	public const CONFIRM_PAGE = 'wapuu-tracker-deactivate';

	/**
	 * admin-post.php / admin_action_* handler key for the destructive submission.
	 */
	public const DELETE_ACTION = 'wapuu_tracker_deactivate_and_delete';
	...
	public function rewrite_deactivate_link( array $actions ): array {
		...
		$actions['deactivate'] = \sprintf(
			'<a href="%s">%s</a>',
			esc_url( $url ),
			esc_html__( 'Deactivate', 'wapuu-tracker' ),
		);
	...
	public function register_hidden_page(): void {
		add_submenu_page(
			'',
			__( 'Deactivate wapuu-tracker', 'wapuu-tracker' ),
			__( 'Deactivate wapuu-tracker', 'wapuu-tracker' ),
	...
	public function render_confirm_page(): void {
		if ( ! current_user_can( $this->current_capability() ) ) {
			wp_die( esc_html__( 'You do not have permission to deactivate this plugin.', 'wapuu-tracker' ), 403 );
		}
	 ...
	 *
	 * @param string $basename Plugin basename, e.g. wapuu-tracker/plugin.php.
	 *
	 * @return string
	 */
	...
	public function handle_destructive(): void {
		if ( ! is_user_logged_in() ) {
			wp_die( esc_html__( 'You must be logged in.', 'wapuu-tracker' ), 403 );
		}
		...
		if ( $confirm !== '1' ) {
			wp_die( esc_html__( 'Confirmation checkbox was not checked.', 'wapuu-tracker' ), 400 );
		}
	 ...
	 *
	 * Example extensions:
	 * - $wpdb->query( "DROP TABLE IF EXISTS {$wpdb->prefix}wapuu_tracker_…" );
	 * - delete_metadata( 'post', 0, 'wapuu_tracker_…', '', true );
	 * - $wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_wapuu_tracker_%'" );
	 *
	...
	protected function cleanup_plugin_data(): void {
		if ( is_multisite() && is_plugin_active_for_network( plugin_basename( Main::file() ) ) ) {
			...
			delete_site_option( 'wapuu_tracker_settings' );
			 ...
			 *         delete_option( 'wapuu_tracker_per_site_settings' );
		...
		delete_option( 'wapuu_tracker_settings' );
	}
}
